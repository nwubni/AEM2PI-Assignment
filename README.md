...
- Cost and latency metrics for every query, logged alongside system responses to `outputs/sample_queries.json`
...
This project requires an OpenAI API key to function. Additionally, it requires the embedding model and LLM model to be specified. Create a `.env` file in the project root directory and add:
```
...
This project includes a test suite to ensure it functions as intended. Use the following command to run the tests:
